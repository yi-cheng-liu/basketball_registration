...
def getFieldPoints():
    ...
    points.append([FIELD_LENGTH - basket_x_shift,
                  FIELD_WIDTH / 2, basket_z_pos])
...
class SVIEWDS(torch.utils.data.Dataset):
    ...
    def getHeatmap(self, img, calib):
        ...
        heatmaps = np.zeros((nbPoints + 1, oriHeight // 4,
                            oriWidth // 4), dtype=np.float32)
    ...
    def __call__(self, key, item):
        ...
        if random.uniform(0, 1) > 0.5:
            ...
            F = np.float32([[-1, 0, width],
                            [0, 1, 0],
                            [0, 0, 1]])
            item.image = cv2.warpAffine(
                item.image, F[0:2, :], (width, height), flags=cv2.INTER_LINEAR)
